...
3. **Promote** (a deliberate, manual step): first snapshot the outgoing `stable` as `v0.<old>` so the retiring line is available for backports, then force-push the candidate onto `stable`. We force-push rather than merge so that `stable`'s history becomes exactly the coherent set of changes made on `main`, superseding whatever adaptations were needed while manually backporting Track 1 fixes onto the old `stable` line. The release-PR job in `release.yml` then opens the `0.(x+1).0` version/changelog PR on `stable`; merging it publishes the breaking release.
...
We use [`release-plz`](https://release-plz.dev) (via the [GitHub Action wrapper](https://github.com/release-plz/action)), configured by [`release-plz.toml`](https://github.com/contentauth/c2patool/blob/main/release-plz.toml). Its two responsibilities run as ordered jobs in [`release.yml`](https://github.com/contentauth/c2patool/blob/main/.github/workflows/release.yml) on release-line branches (`stable` and `v0.*`), never on `main` or release-candidate branches:

* The publish job runs `release-plz release`: when a release PR merges (a push to the release-line branch), it publishes to crates.io, creates a GitHub release, and tags it `v(version)`.
* After publishing completes, the release-PR job runs `release-plz release-pr` from a fresh checkout with full tag history, then opens/updates a **release PR** that bumps the version and updates the changelog. This ordering ensures the version calculation sees any tag created by the publish job, rather than calculating against the previous release and generating an incorrect release PR.

The tag then drives the binary build ([`c2patool-release.yml`](https://github.com/contentauth/c2patool/blob/main/.github/workflows/c2patool-release.yml) on any `v*` tag): `release.yml` does not build binaries itself, which is what lets release-candidate builds produce the same binaries from the same tags (see [RC builds](#release-candidate-builds)). A push whose ref contains `-rc` never publishes to crates.io. Stale release-plz branches are cleaned up only when their PR is closed and only for the same release-line base branch, so work on one line cannot delete another line's active release PR.
