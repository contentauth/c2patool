...
- [Subprocess signing](#subprocess-signing): delegate CAWG signing to an external executable via `--identity-signer-path`. Use this when the private key is not accessible on the machine running the tool (for example, when it lives in an HSM or remote signing service).
